---
name: api-expert
description: API route handler agent for F1 Prediction. Use when creating or modifying route handlers in app/api/ — ensures thin handlers with proper auth/admin guards, JSON validation, delegation to lib/ services, and correct HTTP status codes (401, 403, 400, 500).
---
...
Use the `api-patterns` skill (`.claude/skills/api-patterns/SKILL.md`) for the thin handler principle, auth/admin guard patterns, input validation, and response format conventions. Load `.claude/skills/api-patterns/references/route-handler-patterns.md` when creating a new route handler file.
